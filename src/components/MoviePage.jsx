...
const MoviePage = () => {
  ...
  async function  moviedata(){
      const data1=await fetch("https://api.themoviedb.org/3/movie/"+ movieid+"?language=en-US",TMDB_OPTIONS);
      const json1=await data1.json();
      dispatch(addMovieData(json1))
      const data2=await fetch("https://api.themoviedb.org/3/movie/"+movieid+"/videos?language=en-US",TMDB_OPTIONS);
  ...
  const {poster_path,title,overview,genres,production_companies,release_date,runtime,status  }=data;
  if(vdata===null) return;
  return (
   ...
   <img
     ...
     src={IMG_CDN+poster_path} alt='poster'/>
     <div>
     <h1 className='md:text-4xl font-mono font-bold text-center md:text-left ' >{title}{"("+status+")"}</h1>
     <h1 className='text-2xl pt-2'>Overview</h1>
     <p className='pt-2' >{overview}</p>
     <h1 className='text-2xl pt-2'>genres</h1>
     <p className='pt-2  ' >{genres.map(genera=>"  "+genera.name+", ")} </p>
     <h1 className='text-2xl pt-2'>production companies</h1>
     <ul>
     {production_companies.map(company=><li className='pt-1' >{company.name}</li>)} 
     </ul>
     <h1 className='text-1xl pt-2'> <span className='text-2xl'>Release Date: </span>{release_date}   <br></br> <span className='text-2xl'>Run-Time:</span> {runtime} min</h1>
     </div>
     </div>
     <h1 className='text-3xl pb-2 md:pb-0 pt-4 md:absolute md:pl-8 md:top-[67%] ' >Related Videos</h1>
     <div className='flex gap-4 md:absolute left-3 md:top-[80%] overflow-x-auto w-[93%] bg-black' >
        {
         vdata.results.map(vedio=><div>
         <iframe key={vedio.id} className='pl-4 hover:'
         src={"https://www.youtube.com/embed/"+vedio?.key+"?&controls=0&loop=1"}
